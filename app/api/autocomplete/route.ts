...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const rawAPIRequest = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${body.search.replaceAll(" ", "+")}&key=${process.env.GOOGLE_BOOKS_KEY}&langRestrict=${process.env.GOOGLE_BOOKS_LANG || "en"}&fields=kind,items(id,kind,volumeInfo/title,volumeInfo/authors,volumeInfo/imageLinks)`);
