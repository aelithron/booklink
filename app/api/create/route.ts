...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const dbCheckPreExisting = await db.select().from(bookTable).where(eq(bookTable.googleBooksID, body.id)).limit(1);
  ...
  await db.insert(bookTable).values({ cover: cover, name: book.volumeInfo.title, googleBooksID: body.id, author: authors }).execute();
